            ...
            if (combinedResult[compObj._pdbid]['entities']['resolve']) {
              combinedResult[compObj._pdbid]['entities']['result'][compObj._pdbid].forEach(element => {
                ...
                if (element.entity_id && element.entity_id == 1) {
                  ...
                  // painting quality viewer

                  let outlierDataHash = {};

                  if(combinedResult[compObj._pdbid]["outliers"]["resolve"]) {

                    let outlierResult = combinedResult[compObj._pdbid]["outliers"]["result"][compObj._pdbid]["molecules"];

                    outlierResult.forEach(element => {

                      if(element.entity_id == 1) {
                          ...
                          if(chain.chain_id == compObj._bestChainId) {
                            ...
                            chain.models.forEach(model => {
                              ...
                              model.residues.forEach(residue => {
                                ...
                                if(outlierDataHash[residue.residue_number] == undefined) { 
                                  outlierDataHash[residue.residue_number] = residue.outlier_types;
                                } else {
                                  let tempList = outlierDataHash[residue.residue_number].concat(residue.outlier_types);
                                  outlierDataHash[residue.residue_number] = tempList;
                                }
                    ...
                    for(let incr = 1; incr <= compObj._pdbSequenceLength; incr++) {
                      ...
                      if(outlierDataHash[incr] == undefined) {
                        fragment = {
                          accession: "quality-0", locations: [{
                            fragments: [{
                              ...
                              toolTip: "No validation issue reported for Residue " +incr +" ("  +")"
                            }]
                          }],
                          color: config["color_code"]["quality-0"]
                        };
                          ...
                          accession: "quality-1", locations: [{
                            fragments: [{
                              ...
                              toolTip: "Validation issue: " +outlierDataHash[incr] +
                                "<br>" +"Residue " +incr +" (" +compObj._pdbSequence.charAt(incr) +")"
                            }]
                          }],
                          color: config["color_code"]["quality-1"]
                        };
                        ...
                        fragment = {
                          ...
                          color: config["color_code"]["quality-2"]
                        };
                      } else {
                          fragment = {
                            ...
                            color: config["color_code"]["quality-3"]
                          };
                        }
                      qualityTracks.push(fragment);
                    }

                    compObj.qualityDiv = compObj.mainContent.append('div')
                      .attr('id', 'quality-div');

                    compObj.qualityDiv.append('div')
                      .attr('class', 'left category')
                      .text('Quality');

                    compObj.qualityDiv.append('protvista-track')
                      .attr('id', 'quality-track')
                      .attr('class', 'right')
                      .attr('length', compObj._pdbSequenceLength)
                      .attr('displaystart', compObj._displaystart)
                      .attr('displayend', compObj._displayend);
                    ...
                    document.querySelector('#quality-track').data = qualityTracks;
